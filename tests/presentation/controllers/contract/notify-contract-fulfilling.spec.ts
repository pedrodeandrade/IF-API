...
import { NotifyContractFulfillingMock } from '@/tests/presentation/mocks/use-cases/contract/notify-contract-fulfilling';
import NotifyContractFulfillingController from '@/presentation/controllers/contract/notify-contract-fulfilling';

type SutTypes = {
  sut: NotifyContractFulfillingController
  useCaseSpy: jest.SpyInstance
}

function makeSut() : SutTypes {
  const useCase = new NotifyContractFulfillingMock();

  return {
    useCaseSpy: jest.spyOn(useCase, 'handle'),
    sut: new NotifyContractFulfillingController(useCase),
  };
}
...
describe('NotifyContractFulfilling controller', () => {
  test('it should call NotifyContractFulfillingController with correct values', async () => {
